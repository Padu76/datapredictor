...
export default function UploadPage() {
  ...
  const onData = (data) => {
    ...
    setTarget(''); setDateCol(''); setResult(null);
  };
  ...
  return (
    <div className="container" style={{ paddingTop: 24, paddingBottom: 24 }}>
      <div className="hero card" style={{ padding: 16, marginBottom: 16 }}>
        <h1 className="grad" style={{ margin: 0 }}>Upload & Forecast</h1>
        <p className="hero-sub">Carica un CSV, scegli la colonna target e ottieni un forecast rapido.</p>
      </div>
          ...
          <div className="card" style={{ padding: 16, marginBottom: 16 }}>
            <div className="toolbar">
              ...
              <div style={{ marginLeft: 'auto' }}>
                <button className="primary" onClick={runAnalysis}>Analizza & Prevedi</button>
              </div>
            </div>
            ...
            {stats && (
              <div className="kpis">
                <div>
                  <div className="kpi-title">Min</div>
                  <div className="kpi-value">{stats.min}</div>
                </div>
                <div>
                  <div className="kpi-title">Max</div>
                  <div className="kpi-value">{stats.max}</div>
                </div>
                <div>
                  <div className="kpi-title">Media</div>
                  <div className="kpi-value">{stats.mean}</div>
                </div>
                <div>
                  <div className="kpi-title">Dev. std</div>
                  <div className="kpi-value">{stats.std}</div>
                </div>
              </div>
            )}
          </div>
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
                ...
                <div className="card" style={{ padding: 12 }}>
                  ...
                  <div style={{ color: 'var(--muted)', fontSize: 14, marginTop: 6 }}>{result.insight}</div>
                </div>
              </div>
            </>
          )}
